...
pipeline{% if conclusion_summaries["other_steps"]["has_summary_table"] %}, and
@tbl-final-summary-general shows the general steps' summary{% endif %}.
...
{% if conclusion_summaries["other_steps"]["has_summary_table"] %}
...
def generate_report(**kwargs: Dict[str, Optional[Union[str, int]]]) -> str:
    ...
    # Adding the initial data as step 0 in the statistics (for special cases
    # where there are no new dataset generated by the pipeline,
    # e.g. plate bias)
    if not step_stats:
        step_stats["0"] = {
            "fam": nb_samples,
